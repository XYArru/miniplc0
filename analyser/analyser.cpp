...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseMain() {
		...
		// <常量声明>
        auto errC = analyseConstantDeclaration();
        if (errC.has_value())
            return errC;
		// <变量声明>
        auto errD = analyseVariableDeclaration();
        if (errD.has_value())
            return errD;
		// <语句序列>
        auto errS = analyseStatementSequence();
        if (errS.has_value())
            return errS;
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
        ...
        while (true) {
            ...
            if (next.value().GetType() == TokenType::EQUAL_SIGN) {
				...
				addVariable(vr.value());
				return {};
            }
            // ';'
			else if (next.value().GetType() == TokenType::SEMICOLON) {
				addUninitializedVariable(vr.value());
				return {};
			}
                // wait ...
        }
	...
	std::optional<CompilationError> Analyser::analyseStatementSequence() {
		while (true) {
			...
			case TokenType::IDENTIFIER: {
				...
				_instructions.emplace_back(Operation::STO, stoidx);
				break;
			}
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		if (next.value().GetType() == TokenType::UNSIGNED_INTEGER)
			out	 = std::any_cast<int32_t>(next.value().GetValue()) * flg;
		else {
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrConstantNeedValue);
		}
	...
	std::optional<CompilationError> Analyser::analyseFactor() {
		...
		case TokenType::UNSIGNED_INTEGER: {
			auto vl = std::any_cast<int32_t>(next.value().GetValue());
			_instructions.emplace_back(Operation::LIT, vl);
			return {};
			break;
		}
