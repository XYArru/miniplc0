	...
	std::pair<std::optional<Token>, std::optional<CompilationError>> Tokenizer::nextToken() {
		...
		while (true) {
			...
			case UNSIGNED_INTEGER_STATE: {
                ...
                if(current_state != DFAState::UNSIGNED_INTEGER_STATE)
                    return std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(0, 0, ErrEOF));
                break;
			}
			...
			case IDENTIFIER_STATE: {
				...
				if(gg) {
                    if (ss.str().compare("begin") == 0)
                        return std::make_pair(std::make_optional<Token>(TokenType::BEGIN, ss.str(), pos, currentPos()),
                                              std::optional<CompilationError>());
                    else if (ss.str().compare("end") == 0)
                        return std::make_pair(std::make_optional<Token>(TokenType::END, ss.str(), pos, currentPos()),
                                              std::optional<CompilationError>());
                        ...
                        return std::make_pair(std::make_optional<Token>(TokenType::VAR, ss.str(), pos, currentPos()),
                                              std::optional<CompilationError>());
